        ...
        <header className="h-20 border-b border-white/5 bg-[#020617]/80 backdrop-blur-md sticky top-0 z-30 px-8 flex items-center justify-between">
            <div className="flex items-center gap-6 flex-1">
                ...
                <div className="hidden lg:flex items-center gap-3">
                    {/* Portal Switcher */}
                    <div className="flex items-center bg-white/5 rounded-2xl p-1 border border-white/10 ml-2">
                        {["doctor", "patient", "admin"].map((role) => (
                            <button
                                key={role}
                                onClick={() => setUserRole(role as any)}
                                className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${userRole === role
