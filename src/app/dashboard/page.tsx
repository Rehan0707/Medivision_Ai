...
import { Upload, FileText, ChevronRight, Activity, Bone, AlertCircle, CheckCircle2, Search, Zap, ShieldCheck, Microscope, Scan, Info, Brain, WifiOff, Sparkles, SplitSquareVertical, Database, Users, HeartPulse, Stethoscope, Droplets, Syringe, Pill, ClipboardList, Thermometer, Dna, Waves } from "lucide-react";
...
export default function Dashboard() {
    ...
    return (
        <div className="space-y-10 relative">
            {userRole === "admin" ? (
                <AdminStats t={t} />
            ) : userRole === "patient" && !searchParams.get("modality") && !analysisComplete ? (
                <PatientDashboard t={t} />
            ) : (
                <>
                    {/* Help Overlay */}
                    <AnimatePresence>
                                                    ...
                                                    {group.items.map((m, idx) => (
                                                        ...
                                                        >
                                                            ...
                                                            <div className={`flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.2em] ${m.color} mt-auto`}>
                                                                Initialize Sensor <ChevronRight size={12} />
                                                            </div>
                                                        </motion.div>
                                                    ))}
                                                </div>
